...
class StandaController(object):
    ...
    def close(self):
        self.lib.close_device(ctypes.byref(ctypes.cast(self.device_id, ctypes.POINTER(ctypes.c_int))))

    def get_position(self):
        x_pos = pyximc.get_position_t()
        result = self.lib.get_position(self.device_id, ctypes.byref(x_pos))
        return x_pos.Position, x_pos.uPosition

    def move_absolute(self, distance, udistance):
        result = self.lib.command_move(self.device_id, distance, udistance)
        return self.wait_for_stop()

    def move_relative(self, distance, udistance):
        pos, upos = self.get_position()
        return self.move_absolute(pos + distance, upos + udistance)

    def wait_for_stop(self, interval=100):
        result = self.lib.command_wait_for_stop(self.device_id, interval)
        return repr(result)

    def set_current(self, curr_value):
        eng = pyximc.engine_settings_t()
        result = self.lib.get_engine_settings(self.device_id, ctypes.byref(eng))
        eng.NomCurrent = curr_value
        result = self.lib.set_engine_settings(self.device_id, ctypes.byref(eng))
        return repr(result)

    def set_feedback(self, feedback_type):
        feedback = pyximc.feedback_settings_t()
        self.lib.get_feedback_settings(self.device_id, ctypes.byref(feedback))
        feedback.FeedbackType = feedback_type
        result = self.lib.set_feedback_settings(self.device_id, ctypes.byref(feedback))
        return repr(result)
    ...
    def get_speed(self):
        ...
        result = self.lib.get_move_settings(self.device_id, ctypes.byref(mvst))
    ...
    def set_speed(self, speed):
        ...
        result = self.lib.set_move_settings(self.device_id, ctypes.byref(mvst))
        # Print command return status. It will be 0 if all is OK
        return repr(result)
    ...
    def get_info(self):
        ...
        result = self.lib.get_device_information(self.device_id, ctypes.byref(x_device_information))
        if result == pyximc.Result.Ok:
            return {
    ...
    def get_status(self):
        ...
        result = self.lib.get_status(self.device_id, ctypes.byref(x_status))
        if result == pyximc.Result.Ok:
            return {
    ...
    def get_serial(self):
        ...
        result = self.lib.get_serial_number(self.device_id, ctypes.byref(x_serial))
        if result == pyximc.Result.Ok:
